...
if __name__ == "__main__":
    ...
    out.append(f'<text x="410" y="19" fill="#7d8590" font-size="12" text-anchor="middle">{esc(p["prompt_name"])}@github: ~$ neofetch</text>')
    out.append(f'<text x="28" y="72" fill="#e6edf3" font-size="22" font-weight="700">{esc(p["name"])}</text><text x="28" y="96" fill="#7d8590" font-size="12">{esc(p["username"])}@github</text><line x1="28" y1="112" x2="792" y2="112" stroke="#30363d"/>')
    for i, (label, key, color) in enumerate(ROWS):
        y = 158 + i * 70
        style = "" if static else f' style="animation-delay:{.16+i*.15:.2f}s"'
        out.append(f'<g class="row"{style}><text x="28" y="{y}" fill="{color}" font-size="13" font-weight="700">{label}</text><text x="140" y="{y}" fill="#c9d1d9" font-size="13">{esc(p[key])}</text></g>')
    out.append(f'<line x1="28" y1="570" x2="792" y2="570" stroke="#30363d"/><text x="28" y="610" fill="#7d8590" font-size="12">{esc(p["prompt_name"])}@github:~$ <tspan fill="#c9d1d9">open to connect</tspan></text><rect x="238" y="597" width="8" height="14" fill="#c9d1d9"><animate attributeName="opacity" values="1;1;0;0" keyTimes="0;.5;.51;1" dur="1s" repeatCount="indefinite"/></rect></svg>')
